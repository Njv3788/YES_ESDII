...
class ImageFeedthrough(object):
  ...
  def getStereoHorzontial (self):
    result = self.lib.getFrame(ctypes.c_void_p(self.receiveFrame.ctypes.data))
    return self.receiveFrame[-1:,:,2:4],self.receiveFrame[-1:,:,6:8]
  
  def getStereoVertical (self):
    result = self.lib.getFrame(ctypes.c_void_p(self.receiveFrame.ctypes.data))
    return self.receiveFrame[:,-1:,0:2],self.receiveFrame[:,-1:,4:6]
